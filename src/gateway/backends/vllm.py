"""vLLM backend adapter.

vLLM natively exposes an OpenAI-compatible API at /v1/chat/completions.
Reference: https://docs.vllm.ai/en/latest/serving/openai_compatible_server.html
"""

from __future__ import annotations

import json
import logging
from typing import AsyncIterator

import httpx

from gateway.backends.base import BackendFactory, BaseHTTPBackend
from gateway.models.request import ChatRequest
from gateway.models.response import (
    ChatChoice, ChatChoiceMessage, ChatChunk, ChatChunkChoice,
    ChatChunkDelta, ChatResponse, UsageInfo,
)

logger = logging.getLogger(__name__)


@BackendFactory.register("vllm")
class VLLMBackend(BaseHTTPBackend):
    """Backend adapter for vLLM inference server."""

    async def complete(self, request: ChatRequest) -> ChatResponse:
        payload = request.to_backend_payload()
        payload["stream"] = False
        try:
            resp = await self._client.post("/v1/chat/completions", json=payload)
            resp.raise_for_status()
            return self._parse_response(resp.json())
        except httpx.TimeoutException as e:
            logger.error("vLLM %s timed out: %s", self._name, e)
            raise
        except httpx.HTTPStatusError as e:
            logger.error("vLLM %s returned %d: %s", self._name,
                        e.response.status_code, e.response.text[:500])
            raise
        except httpx.HTTPError as e:
            logger.error("vLLM %s HTTP error: %s", self._name, e)
            raise

    async def stream(self, request: ChatRequest) -> AsyncIterator[ChatChunk]:
        payload = request.to_backend_payload()
        payload["stream"] = True
        try:
            async with self._client.stream("POST", "/v1/chat/completions", json=payload) as resp:
                resp.raise_for_status()
                async for line in resp.aiter_lines():
                    if not line or not line.startswith("data: "):
                        continue
                    data_str = line[6:]
                    if data_str.strip() == "[DONE]":
                        return
                    try:
                        yield self._parse_chunk(json.loads(data_str))
                    except json.JSONDecodeError:
                        logger.warning("vLLM %s malformed chunk: %s", self._name, data_str[:200])
                        continue
        except httpx.HTTPError as e:
            logger.error("vLLM %s stream error: %s", self._name, e)
            raise

    async def health(self) -> bool:
        try:
            resp = await self._client.get("/health", timeout=5.0)
            return resp.status_code == 200
        except (httpx.HTTPError, Exception):
            return False

    async def list_models(self) -> list[str]:
        try:
            resp = await self._client.get("/v1/models", timeout=5.0)
            if resp.status_code == 200:
                return [m["id"] for m in resp.json().get("data", [])]
        except (httpx.HTTPError, Exception) as e:
            logger.debug("vLLM %s model discovery failed: %s", self._name, e)
        return self._models

    def _parse_response(self, data: dict) -> ChatResponse:
        try:
            choices = [
                ChatChoice(
                    index=c.get("index", 0) if isinstance(c, dict) else 0,
                    message=ChatChoiceMessage(
                        content=c.get("message", {}).get("content") if isinstance(c, dict) else None
                    ),
                    finish_reason=c.get("finish_reason") if isinstance(c, dict) else None,
                ) for c in data.get("choices", [])
            ]
            u = data.get("usage") or {}
            return ChatResponse(
                id=data.get("id", ""), model=data.get("model", ""), choices=choices,
                usage=UsageInfo(prompt_tokens=u.get("prompt_tokens", 0),
                               completion_tokens=u.get("completion_tokens", 0),
                               total_tokens=u.get("total_tokens", 0)),
                backend=self._name,
            )
        except Exception as e:
            logger.error("vLLM %s parse error: %s", self._name, e)
            return ChatResponse(model=data.get("model", ""), backend=self._name)

    def _parse_chunk(self, data: dict) -> ChatChunk:
        try:
            choices = [
                ChatChunkChoice(
                    index=c.get("index", 0) if isinstance(c, dict) else 0,
                    delta=ChatChunkDelta(
                        role=c.get("delta", {}).get("role") if isinstance(c, dict) else None,
                        content=c.get("delta", {}).get("content") if isinstance(c, dict) else None,
                    ),
                    finish_reason=c.get("finish_reason") if isinstance(c, dict) else None,
                ) for c in data.get("choices", [])
            ]
            return ChatChunk(id=data.get("id", ""), model=data.get("model", ""),
                             choices=choices, backend=self._name)
        except Exception as e:
            logger.error("vLLM %s chunk parse error: %s", self._name, e)
            return ChatChunk(model=data.get("model", ""), backend=self._name)
